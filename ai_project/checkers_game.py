...
class CheckersGame:
            ...
            for col in range(BOARD_SIZE):
                ...
                if self.selected == (row, col):
                    self.canvas.create_rectangle(col*SQUARE_SIZE, row*SQUARE_SIZE,
                                                 ...
                                                 fill=SELECT_COLOR)
                if piece:
                    self.canvas.create_oval(col*SQUARE_SIZE+10, row*SQUARE_SIZE+10,
                                           (col+1)*SQUARE_SIZE-10, (row+1)*SQUARE_SIZE-10,
                                           fill=P1_COLOR if piece.player == 1 else P2_COLOR)
                    if piece.king:
                        self.canvas.create_oval(col*SQUARE_SIZE+20, row*SQUARE_SIZE+20,
    ...
    def get_moves_for_piece(self, row, col, piece):
        ...
        for dr, dc in directions:
            ...
            if (0 <= jump_row < BOARD_SIZE and 0 <= jump_col < BOARD_SIZE and
                self.board[new_row][new_col] and self.board[new_row][new_col].player != piece.player and
    ...
    def make_move(self, from_pos, to_pos):
        ...
        # Kinging
        if (piece.player == 1 and tr == 0) or (piece.player == 2 and tr == BOARD_SIZE-1):
            piece.king = True
        # Capture
        if abs(tr - fr) == 2:
            mid_row = (fr + tr)//2
